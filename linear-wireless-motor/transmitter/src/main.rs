...
async fn main(spawner: Spawner) {
    ...
    let buttons = [
        Button::new(AnyInput::new(io.pins.gpio6, Pull::Up), Direction::Left),
        // Button::new(AnyInput::new(io.pins.gpio10, Pull::Up), Colour::Yellow),
        Button::new(AnyInput::new(io.pins.gpio5, Pull::Up), Direction::Right),
    ];
    ...
    for button in buttons {
        info!("{} button ready.", button.direction);
...
async fn broadcaster(
    ...
    loop {
        ...
        trace!("{} button pressed", button.direction);
        ...
        match sender
            .send_async(&BROADCAST_ADDRESS, &[button.direction.to_u8()])
            .await
        {
            Ok(()) => info!("{} button press sent", button.direction),
            Err(e) => warn!("Couldn't send {} button press: {e:?}", button.direction),
        }
